function CSG_Inverse_Kinematics
    ...
    pE = [0; 0; 0.4];  
    bend = 0*pi/180;  
    RE = [cos(bend) 0 sin(bend);0 1 0;-sin(bend) 0 cos(bend)];
    %Hole Pattern (vector operations)
    scrib_R = 0.087;
    alpha1 = 100*pi/180;  
    alpha2 = 120*pi/180 - alpha1; 
    i = 1:6;
    ...
    inv_hat_xy=@(y)[y(3,2);y(1,3)];
    
    %Main Simulation
    init_guess = [zeros(30,1); pE(3)*ones(6,1)]; %Initial guess
    global p
    options = optimoptions('fsolve','display','iter-detailed')
    fsolve(@CSG_BVP_Function, init_guess,options); %Solve CSG BVP with shooting method
    ...
    function E = CSG_BVP_Function(G)
        ...
        for i = 1 : 6 %Loop over each compliant robot link
            n0_range = 1+5*(i-1) : 3+5*(i-1);   
            m0_range = 4+5*(i-1) : 5*i;
            n0 = G(n0_range);  
            m0 = [G(m0_range); 0];  
            L = G(30+i);
