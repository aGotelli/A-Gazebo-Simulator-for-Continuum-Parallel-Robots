...
/*! Visualizes an x and y series. The thread of execution is paused while the window is open. */
static void plot(VectorXd x1, VectorXd y1, VectorXd x2, VectorXd y2, QString title = "", QString x_title = "", QString y_title = ""){
    if(x1.size()!=y1.size()){
        QMessageBox msgBox;
        msgBox.setWindowTitle("Plotting Error");
        msgBox.setText("x and y vectors must be the same size.");
    ...
    QtCharts::QLineSeries* p1 = new QtCharts::QLineSeries(chart);
    for (int i = 0; i < x1.size(); i++)
        p1->append(QPointF(x1(i), y1(i)));
    chart->addSeries(p1);
    QtCharts::QLineSeries* p2 = new QtCharts::QLineSeries(chart);
    for (int i = 0; i < x2.size(); i++)
        p2->append(QPointF(x2(i), y2(i)));
    chart->addSeries(p2);
